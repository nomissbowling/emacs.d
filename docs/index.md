...
* 設定ファイルは全て `~/Dropbox/emacs.d/` に置いて Git管理し、
[init.el](https://github.com/minorugh/emacs.d/blob/master/init.el) のシンボリックを `~/.emacs.d` に置いて複数端末から共有しています。
...
私の init.el は、インストールパッケージ数 110 + 必要最小限の内容ですので、遅延処理しなくても 2 秒以下で起動しますが、この `after-init-hook` の処理で 0.5 秒程度、起動時間を短縮できています。

からくり GC 設定のほうは、`after-init-hook` が実行されたあとに `emacs-startup-hook` が実行されるという仕組みを利用してわけですが、0.7 秒ほど改善されています。
...
### 5.3 viewモード
elisp , perl , html 等々のソースファイルを見るときは、`view-mode` を使っています。
...
![hydra-view-mode](https://live.staticflickr.com/65535/50174801848_c99c7f367e_b.jpg)
