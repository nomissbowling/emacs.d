...
ファイルの PATH は、ご自分の環境に応じて修正が必要です。Packageテスやエラーで Emacsが起動しない場合に使用します。
...
### 3.5 初期画面設定
Emacs起動時の初期画面表示のための設定ファイル [init-config.el](https://github.com/minorugh/emacs.d/blob/master/init-config.el) を遅延読み込みより前に先読み込みします。（前項の設定を参照）
